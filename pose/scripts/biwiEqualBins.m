function [sRmse, sMae] = biwiEqualBins(seed, params)
...
centroids = [c(1:s:end)', c(2:s:end)', c(3:s:end)'];
...
for n = 1:3
  [y, yt] = ...
    centroidDiscretizations(YTrain(:, n), centroids, N, M, YTest(:, n));
...
for i =1:length(YPredicted)
  for n = 1:3
    ...
    YPredicted(i, n) = mean(sum(centroids .* currY));
  end
end
